...
#' simplify downloading observations for a specific user, filtered by year and
#' optionally by month and day.
#' @param user_id The numeric ID or login of the Minka user.
...
#' @param quiet A logical value. If `TRUE`, all console messages will be
#' suppressed.
#' @param limit_download A logical value. If `TRUE` (default), the download is
#' capped at 10,000 records per query subdivision. If `FALSE`, it attempts to
#' download all records.
#' @return A `tibble::tibble` containing the downloaded observation data.
#' @export
#' @examples
#' \dontrun{
#' # Download all observations for user ramonaervitje (user_id = 6) for the year
#' 2024
...
#' user_data_aug_2025 <- mnk_user_obs(user_id = 5678, year = 2025, month = 8,
#' limit_download = FALSE)
#' }
mnk_user_obs <- function(user_id, year, month = NULL, day = NULL, quiet = FALSE, limit_download = TRUE) {
