## rminka 0.1.0 (2026-03-11)  
### Nuevas funcionalidades 
*   Se añadió la función `load_marine_data()` para cargar datos de sensores.
*   Implementada la función `analyze_species()` para análisis de diversidad.
### Cambios importantes
*   La función `plot_data()` ahora usa `ggplot2` por defecto.
### Correcciones de errores
*   Corregido un error en `calculate_depth()` que causaba valores negativos.
## rminka 0.0.1 (2026-02-28) # <-- También con '##' para esta versión
*   Versión inicial del paquete
